...
const App = () => {
  ...
  const datasorting = employee => {
    ...
    const value = [
      {
        day: 'today',
        data: employee.filter(x => x.date == todayformat),
      },
      {
        day: 'yesterday',
        data: employee.filter(x => x.date == yesterday),
      },
      {
        day: 'Previous',
        data: employee.filter(
          x => x.date !== todayformat && x.date !== yesterday,
        ),
      },
    ];
    ...
    //   {
    //     day: 'yesterday',
    //     data: employee.filter(x => x.tool == data.tool),
    //   },
    // ];
    // setlist(value1);
    setData(value);
